...
apply-services: ## Apply services layer (S3 buckets, observability, applications)
	...
	@bash -c 'set -euo pipefail; \
	  ...
	  # Preserve OVH state-backend keys (production). Local has no remote S3 state. \
	  STATE_AWS_ACCESS_KEY_ID="$${AWS_ACCESS_KEY_ID:-}"; \
	  STATE_AWS_SECRET_ACCESS_KEY="$${AWS_SECRET_ACCESS_KEY:-}"; \
	  eval $$(vault kv get -format=json secret/rgw/credentials | jq -r ".data.data | \"export RGW_ACCESS_KEY=\(.access_key); export RGW_SECRET_KEY=\(.secret_key)\""); \
	  echo "Applying S3 buckets and remaining infrastructure..."; \
	  TF_DIR="iac/envs/$(ENV)"; \
	  # Local smoke: Velero target is in-cluster RGW (reuse RGW keys). Production: keep OVH keys from tfvars. \
	  if [ "$(ENV)" = "local" ]; then \
	    export AWS_ACCESS_KEY_ID="$$RGW_ACCESS_KEY" AWS_SECRET_ACCESS_KEY="$$RGW_SECRET_KEY"; \
	    export TF_VAR_backup_s3_access_key="$$RGW_ACCESS_KEY" TF_VAR_backup_s3_secret_key="$$RGW_SECRET_KEY"; \
	  else \
	    export AWS_ACCESS_KEY_ID="$$STATE_AWS_ACCESS_KEY_ID" AWS_SECRET_ACCESS_KEY="$$STATE_AWS_SECRET_ACCESS_KEY"; \
	    export TF_VAR_rgw_s3_access_key="$$RGW_ACCESS_KEY" TF_VAR_rgw_s3_secret_key="$$RGW_SECRET_KEY"; \
	  fi; \
